...
import Addqualitystandard from './Addqualitystandard';
...
function Qualitystandardmaster(props) {

  const [modalopen,setModalopen] = useState(false)
  const [deletemodalopen,setDeleteodalopen] = useState(false)
  const [qualitystandarddetails,setqualitystandarddetails] = useState([]);
  const [qualitystandarddetailsfiltered,setqualitystandarddetailsfiltered] = useState([]);
  const [updateid,setUpdateid] = useState('');
  const [deleteid,setDeleteid] = useState('');
  const [search,setSearch] = useState("");

  const keys = ["qualitytype","details"];
  
  useEffect(() => {
    const result = qualitystandarddetails.filter(item => {
      return keys.some((key) => (item[key]===null?"":item[key]).toLowerCase().includes(search.toLowerCase()))
    })
    setqualitystandarddetailsfiltered(result)
  }, [search])

  useEffect(()=>{
    QualityStandardmaster_service.getAll().then((response)=>{
       setqualitystandarddetails(response.data);
       console.log(response.data)
       setqualitystandarddetailsfiltered(response.data)
...
return (
     <Box>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} alignItems={"center"} style={{textAlign:"center"}}>
        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} alignItems={"center"}  className='pageheader'>
          ...
          <div className='masterHeading'><h2 className='centerContent' style={{color:'white',fontSize: '24px'}}> Quality Parameter Master </h2> </div>
          </Grid>
          <Grid item xs={5}>
              <Addqualitystandard open={modalopen} onClose={handlemodalclose} updateid={updateid} />

            <Deletecomponent open={deletemodalopen} onClose={handledeletemodalclose} deleteid={deleteid} servicename="deleteQualitybyid" />
  ...
  <DataTable
            columns={columns}
            data={qualitystandarddetailsfiltered}
            fixedHeader
...
export default Qualitystandardmaster;