...
function Addreason(props) {
  ...
  useEffect(()=>{
    if(props.updateid){
    Reasonmaster_service.getbyid(props.updateid).then((res)=>{
  ...
  const submitbutton = ()=>{
    ...
    if(fieldempty){
      return <Button size="large" variant="contained" onClick={reasonmasterdetails} style={{marginBottom:"20px"}} >
            Submit
            </Button>           
    }
else return "";
  }
  const reasonmasterdetails = (e) =>{
    e.preventDefault();
    setLoading(true);
    let entryby = sessionStorage.getItem("entryby")  
     ...
     if(props.updateid){
      ...
      console.log(details);
      Reasonmaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {name,stname,entryby};
      console.log(details);
      Reasonmaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
  ...
  <Box sx={style}>
    ...
    >
      ...
      label="Reason Name" variant="standard" 
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Reason Short Name" variant="standard" 
