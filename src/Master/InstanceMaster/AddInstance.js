...
function Addinstance(props) {
  ...
  const [instancestname,setinstancestname] = useState('');
  ...
  useEffect(()=>{
    if(props.updateid){
    Instancemaster_service.getbyid(props.updateid).then((res)=>{
      setpenalty(res.data.penalty);
      setinstancename(res.data.instancename);
      setinstancestname(res.data.instancestname);
  ...
  const submitbutton = ()=>{
    const fieldempty =  instancename && instancestname && penalty;
    if(fieldempty){
      return <Button size="large" variant="contained" onClick={instancemasterdetails} style={{marginBottom:"20px"}} >
            Submit
            </Button>           
    }
else return "";
  }
  const instancemasterdetails = (e) =>{
    e.preventDefault();
    setLoading(true);
    let entryby = sessionStorage.getItem("entryby")  

     if(props.updateid){
      const details = {instancename,instancestname,penalty,entryby};
      console.log(details);
      Instancemaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {instancename,instancestname,penalty,entryby};
      console.log(details);
      Instancemaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
  ...
  <Box sx={style}>
    ...
    >
      ...
      label="Instance Name" variant="standard" 
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Instance Short Name" variant="standard" 
      name='instancestname' value={instancestname} 
      onChange={(e)=>setinstancestname(e.target.value)} 
      required
        >
        </TextField>
        <TextField id="standard-basic" 
