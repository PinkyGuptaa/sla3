...
function AddIncentive(props) {
  ...
  useEffect(()=>{
    if(props.updateid){
    Incentivemaster_service.getbyid(props.updateid).then((res)=>{
  ...
  const submitbutton = ()=>{
    const fieldempty = slaMaster && qualityStandardMaster && incentive ;
    if(fieldempty){
  ...
  const penaltymasterdetails = (e) =>{
    ...
    let entryby = sessionStorage.getItem("entryby")  
     ...
     if(props.updateid){
      const details = {  slaMaster,qualityStandardMaster,incentive,entryby};
      console.log(details);
      Incentivemaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {  slaMaster,qualityStandardMaster,incentive,entryby};
      console.log(details);
      Incentivemaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
...
select
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Incentive" variant="standard" 
