...
function Addaction(props) {
  ...
  useEffect(()=>{
    if(props.updateid){
    Actionmaster_service.getbyid(props.updateid).then((res)=>{
  ...
  const submitbutton = ()=>{
    ...
    if(fieldempty){
      return <Button size="large" variant="contained" onClick={actionmasterdetails} style={{marginBottom:"20px"}} >
            Submit
            </Button>           
    }
else return "";
  }
  const actionmasterdetails = (e) =>{
    e.preventDefault();
    setLoading(true);
    let entryby = sessionStorage.getItem("entryby")  
     ...
     if(props.updateid){
      ...
      console.log(details);
      Actionmaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {  name,stname,entryby};
      console.log(details);
      Actionmaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
  ...
  <Box sx={style}>
    ...
    >
      ...
      label="Action Name" variant="standard" 
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Action Short Name" variant="standard" 
