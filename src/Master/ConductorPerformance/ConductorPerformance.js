...
function ConductorPerformance(props) {

  const [modalopen,setModalopen] = useState(false)
  const [deletemodalopen,setDeleteodalopen] = useState(false)
  const [Conductordetails,setConductordetails] = useState([]);
  const [Conductordetailsfiltered,setConductordetailsfiltered] = useState([]);
  const [updateid,setUpdateid] = useState('');
  const [deleteid,setDeleteid] = useState('');
  const [search,setSearch] = useState("");
  const keys = ["details", "penaltydetail"];
  
  useEffect(() => {
    const result = Conductordetails.filter(item => {
      return keys.some((key) => (item[key]===null?"":item[key]).toLowerCase().includes(search.toLowerCase()))
    })
    setConductordetailsfiltered(result)
  }, [search])

  useEffect(()=>{
    Conductor_service.getAll().then((response)=>{

       console.log(response.data);
       setConductordetails(response.data);
       setConductordetailsfiltered(response.data)
        ...
        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} alignItems={"center"} style={{textAlign:"center",background:"lightgrey"}}>
          <Grid item xs={3}>
            <Button size='large' variant="contained" startIcon={<AddIcon />} onClick={handlemodal}>
                Add Conductor Metrics 
            </Button>
          </Grid>
          <Grid item xs={6}>
              <AddConductor open={modalopen} onClose={handlemodalclose} updateid={updateid} />

            <Deletecomponent open={deletemodalopen} onClose={handledeletemodalclose} deleteid={deleteid} servicename="deleteConductorbyid" />
            <div className='masterHeading'><h2 className='centerContent'> Conductor Performance Metrics </h2> </div>
          </Grid>
          <Grid item xs={3}>
