...
function Addgenericpenalty(props) {
  ...
  useEffect(()=>{
    if(props.updateid){
    Genericpenaltymaster_service.getbyid(props.updateid).then((res)=>{
    
      setname(res.data.name);
      setstname(res.data.stname);
      setpenaltypercentage(res.data.penaltypercentage)
  ...
  const submitbutton = ()=>{
    const fieldempty =  name && stname &&  penaltypercentage;
    if(fieldempty){
      return <Button size="large" variant="contained" onClick={genericmasterdetails} style={{marginBottom:"20px"}} >
            Submit
            </Button>           
    }
else return "";
  }
  const genericmasterdetails = (e) =>{
    e.preventDefault();
    setLoading(true);
    let entryby = sessionStorage.getItem("entryby")  

     if(props.updateid){
      const details = {name,stname,penaltypercentage,entryby};
      console.log(details);
      Genericpenaltymaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {name,stname,penaltypercentage,entryby};
      console.log(details);
      Genericpenaltymaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
  ...
  <Box sx={style}>
    ...
    >
      ...
      label="Penalty Name" variant="standard" 
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Penalty Short Name" variant="standard" 
...
export default Addgenericpenalty;