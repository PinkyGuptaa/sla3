...
function GenericPenaltyMaster(props) {
  ...
  const [genericdetailsfiltered,setgenericdetailsfiltered] = useState([]);
  ...
  useEffect(() => {
    const result = genericdetails.filter(item => {
      return keys.some((key) => (item[key]===null?"":item[key]).toLowerCase().includes(search.toLowerCase()))
    })
    setgenericdetailsfiltered(result)
  }, [search])

  useEffect(()=>{
    Genericpenaltymaster_service.getAll().then((response)=>{
       console.log(response.data);
       setgenericdetails(response.data);
       setgenericdetailsfiltered(response.data)
     ...
     <Box>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} alignItems={"center"} style={{textAlign:"center"}}>
        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} alignItems={"center"}  className='pageheader'>
          ...
          <div className='masterHeading'><h2 className='centerContent' style={{color:'white',fontSize: '24px'}}> Generic Penalty Master </h2> </div>
          </Grid>
          <Grid item xs={5}>
            <AddGenericpenalty open={modalopen} onClose={handlemodalclose} updateid={updateid} />
            <Deletecomponent open={deletemodalopen} onClose={handledeletemodalclose} deleteid={deleteid} servicename="deletegenericbyid" />
           
          </Grid>
          <Grid item xs={3} style={{marginTop:"10px "}}>
          <Button style={{backgroundColor:'#136a8a', marginRight:'50px'}} variant="contained" startIcon={<AddIcon />} onClick={handlemodal}>
                Add Generic Penalty
            </Button>
          </Grid>
        </Grid>
...
export default GenericPenaltyMaster;