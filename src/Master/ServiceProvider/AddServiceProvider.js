...
useEffect(() => {
  const selectedSlaDetails = sladetails.find((option) => option.slaMaster.sla === sla);

  if (selectedSlaDetails) {
    setdetails(selectedSlaDetails.details);
    setinstance(selectedSlaDetails.instance);
    setpenalty(selectedSlaDetails.penalty);
    setpenaltydetail(selectedSlaDetails.penaltydetail);
    setQualityType(selectedSlaDetails.qualityStandardMaster.qualitytype)
    setfiledate(selectedSlaDetails.filedate)
  }
}, [sla, sladetails]);
...
const getSLABus = async (id) => {
  try {
    const response = await axios.get(`http://10.226.33.132:9100/busperformance/getbyisresolvee`);
...
const getSLAConductor = async (id) => {
  try {
    const response = await axios.get(`http://10.226.33.132:9100/conductorperformance/getbyisresolvee`);
    return response.data;
  } catch (error) {
...
const getSLADriver = async (id) => {
  try {
    const response = await axios.get(`http://10.226.33.132:9100/driverperformance/getbyisresolvee`);
    return response.data;
  } catch (error) {
...
const submitbutton = () => {
  ...
  const busmasterdetails = (e) =>{
    ...
    axios.post(`http://10.226.33.132:9100/busperformance/setacceptflag/${sla}`).then((res)=>{
            props.onClose();
