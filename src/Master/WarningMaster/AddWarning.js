...
function Addwarning(props) {
  ...
  useEffect(()=>{
    if(props.updateid){
    Warningmaster_service.getbyid(props.updateid).then((res)=>{
  ...
  const submitbutton = ()=>{
    ...
    if(fieldempty){
      return <Button size="large" variant="contained" onClick={warningmasterdetails} style={{marginBottom:"20px"}} >
            Submit
            </Button>           
    }
else return "";
  }
  const warningmasterdetails = (e) =>{
    e.preventDefault();
    setLoading(true);
    let entryby = sessionStorage.getItem("entryby")  
     ...
     if(props.updateid){
      ...
      console.log(details);
      Warningmaster_service.update(props.updateid,details).then((res)=>{
        props.onClose();
     ...
     else 
     {
      ...
      const details = {name,stname,entryby};
      console.log(details);
      Warningmaster_service.add(details).then((res)=>{
        props.onClose();
        setLoading(false);
        setSnackcolor("#458a32");
  ...
  <Box sx={style}>
    ...
    >
      ...
      label="Warning Name" variant="standard" 
      ...
      required
        ...
        <TextField id="standard-basic" 
      label="Warning Short Name" variant="standard" 
